...
import { BuildStep, TemplateResponse, ChatMessage, FileStructure } from '../Types';
...
function BuildView() {
  ...
  useEffect(() => {
    if (editorInstance && streamingState.isStreaming && streamingState.currentFile) {
    ...
    async function init() {
      ...
      try {
        ...
        await axios.post(`${BACKEND_URL}/chat`, {
          message: [...prompts, prompt].map((content: string): ChatMessage => ({ parts: content }))
        });
  ...
  const handleFileSelect = (code: string, path?: string) => {
    ...
    let filePath = path;
    if (!filePath) {
      const streamingFile = Object.values(streamingState.files).find(f => f.content === code);
      if (streamingFile) {
  ...
  const handleStreamingChunk = (data: any) => {
    ...
    if (event === 'artifact_start') {
      ...
      const filePath = artifact.path;
      console.log('📝 Content chunk for:', filePath, 'Content length:', content?.length);
  ...
  const buildFileStructure = (artifacts: { path: string; code: string }[]): FileStructure => {
  ...
  const handleCompleteArtifact = (artifact: any, stepNumber: number) => {
    ...
    setBuildSteps(prev => {
      ...
      const allArtifacts = newSteps.map(step => ({ path: step.path, code: step.code }));
